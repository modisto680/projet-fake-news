...
with DAG(
    ...
    catchup=False,
    max_active_runs=1,
    tags=["bluesky", "mongo", "kedro", "nlp", "tfidf"],
) as dag:
    login = BashOperator(
    ...
    preprocess_and_vectorize = BashOperator(
        ...
        cwd=KEDRO_DIR,
        # "__default__" = preprocessing + tfidf (voir pipeline_registry.py)
        bash_command="kedro run",
    )

    login >> collect_raw >> preprocess_and_vectorize
