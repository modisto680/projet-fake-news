...
def load_raw_posts_from_mongo(
    ...
    raw_cols = mongo_params.get("raw_collections") or []
    # compat: ancien param raw_collection
    if not raw_cols and mongo_params.get("raw_collection"):
        raw_cols = [mongo_params["raw_collection"]]

    if not raw_cols:
        raise ValueError("mongo.raw_collections manquant dans parameters.yml")
    ...
    for raw_col in raw_cols:
        col = db[raw_col]
        batch = list(col.find({}, projection=projection))
        for d in batch:
            d["_raw_collection"] = raw_col
        docs.extend(batch)
