...
def load_posts_processed_from_mongo(
    ...
    """
    Charge les posts depuis MongoDB (collection cleaned_posts) dans un DataFrame.
    """
...
def save_tfidf_artefacts(
    ...
    """
    Sauvegarde le vectorizer (et svd si utilisé) dans data/07_model_output/ via joblib.
    Retourne les chemins sauvegardés.
    """
    # Chemin consolidé dans Kedro — couvert par le volume Docker ./kedro
    out_dir = "/opt/airflow/kedro/facts_check/data/07_model_output"
    ...
    if artefacts.get("svd") is not None:
        svd_path = os.path.join(out_dir, "tfidf_svd.joblib")
...
def save_posts_tfidf_to_mongo(
    ...
    On stocke les vecteurs svd_* (petit vecteur dense) pour alimenter un modèle IA.
    """
    db_name = mongo_params["db"]
